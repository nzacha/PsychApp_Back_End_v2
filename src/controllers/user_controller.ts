...
export async function createUser(request: express.Request, response: express.Response){
    try{
        ...
        if(user){
            sendMail(email, 'PsychApp Registration', undefined, `<h1>Registration to PsychApp</h1><h2>Welcome ${user.first_name} ${user.last_name}!</h2><p>Your password is: <b>${password}</b></p><p>use the following link to acces the page: <a>${process.env.WEB_HOST}</a></p>`);
